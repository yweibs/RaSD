...
def GaussianBlur(x, sigma=None, level=None, random=False, min_sigma=0, isotropic=False, device='cpu'):
    ...
    for i in range(n_dims):
        ...
        x = nnf.conv2d(x, k, padding=(*zeros[:i], k.shape[i+2]//2, *zeros[i+1:]), stride=1, dilation=1)
...
def GaussianNoise(x, noise_min=0.01, noise_max=0.10, noise_only=False, absolute=False, axes=(0, -1), device='cpu'):
    ...
    return noise if noise_only else x + noise
...
def random_blur_rescale(x, std_min=8 / 2.355, std_max=32 / 2.355, isotropic=False, reduce=torch.std, device='cpu'):
    ...
    rand = np.random.default_rng() # 创建随机数生成器
    seeds = rand.integers(np.iinfo(int).max, size=n_dim) # 为每一个维度生成一个随机种子，用于创建高斯核
    ...
    after = reduce(x) # std
    return x * before/(after+1e-6) # 调整std
...
if __name__ == '__main__':
    ...
    lab = transform(lab, flow)

    lab = torch.argmax(lab, dim=0, keepdim=True).type(torch.float32)
    ...
    flow = draw_perlin(out_shape=(1, num_dim, *in_shape),
                       scales=(8,),
                       min_std=0,
                       max_std=16, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    # flow = spatial_aug.rand_coords(lab.shape[2:])
    # flow = torch.from_numpy(flow)
    # if torch.cuda.is_available():
    #     flow = flow.cuda()

    warpped_lab = transform(lab, flow, mode="nearest")

    # image_1 = labels_to_image(warpped_lab, num_label, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    # image_1 = sitk.GetImageFromArray(image_1.data.cpu().numpy()[0, 0])
    # sitk.WriteImage(image_1, 'test_img_1.nii')

    # image_2 = labels_to_image(warpped_lab, num_label, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    # image_2 = sitk.GetImageFromArray(image_2.data.cpu().numpy()[0, 0])
    # sitk.WriteImage(image_2, 'test_img_2.nii')

    # image_3 = labels_to_image(warpped_lab, num_label, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    # image_3 = sitk.GetImageFromArray(image_3.data.cpu().numpy()[0, 0])
    # sitk.WriteImage(image_3, 'test_img_3.nii')

    # image_4 = labels_to_image(warpped_lab, num_label, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    # image_4 = sitk.GetImageFromArray(image_4.data.cpu().numpy()[0, 0])
    # sitk.WriteImage(image_4, 'test_img_1.nii')

    # image_5 = labels_to_image(warpped_lab, num_label, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    # image_5 = sitk.GetImageFromArray(image_5.data.cpu().numpy()[0, 0])
    # sitk.WriteImage(image_5, 'test_img_5.nii')

    

    warpped_lab = sitk.GetImageFromArray(warpped_lab.data.cpu().numpy()[0, 0])
    sitk.WriteImage(warpped_lab, 'test_lab.nii')
    lab = sitk.GetImageFromArray(lab.data.cpu().numpy()[0, 0])
    sitk.WriteImage(lab, 'test_lab_org.nii')
