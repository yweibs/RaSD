...
def draw_perlin(out_shape, scales, min_std=0, max_std=1, dtype=torch.float32, device='cpu'):
    ...
    for scale in scales:
        ...
        # noise_frames.append(gauss.clone())  # 保存当前步骤的噪声图
        out += gauss

    # noise_frames.append(out.clone())  # 保存最终的噪声图
    return out
...
def save_noise_frames(noise_frames, base_path, scales):
    for i, frame in enumerate(noise_frames):
        ...
        frame = frame.squeeze()  # 去除批次维度
        frame = (frame - frame.min()) / (frame.max() - frame.min())  # 归一化到[0, 1]
        frame = frame.cpu().numpy()  # 转换为NumPy数组
        pil_img = Image.fromarray((frame * 255).astype(np.uint8))  # 转换为PIL图像
        pil_img.save(filepath)  # 保存图像
...
if __name__ == '__main__':
    ...
    out, noise_frames = draw_perlin(out_shape=[num_label, 1, *in_shape], scales=[32, 64, 128], max_std=1, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    out_2, noise_frames_2 = draw_perlin(out_shape=[num_label, num_dim, *in_shape], scales=[32, 64, 128], max_std=16, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    
    base_path = './img'
    
    if not os.path.exists(base_path):
        os.makedirs(base_path)
    save_noise_frames(noise_frames, base_path, scales=[32, 64, 128])
    
    warpped_im = transform(noise_frames, noise_frames_2)
