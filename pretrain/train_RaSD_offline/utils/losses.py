...
def app_gradient_loss(mask, s, penalty='l2'):
    ...
    d = torch.mean(dx*mask_x) + torch.mean(dy*mask_y) + torch.mean(dz*mask_z)
    return d / 3.0
...
def ncc_loss(I, J, win=None):
    ...
    sum_filt = torch.ones([1, I.shape[1], *win]).to("cuda")
...
def partical_MAE(y_true, y_pred, mask):
    return torch.sum(torch.abs(y_true - y_pred) * mask)/(torch.sum(mask)+1e-6)
