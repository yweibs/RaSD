...
def random_blur_rescale(x, std_min=8 / 2.355, std_max=32 / 2.355, isotropic=False, reduce=torch.std, device='cpu'):
    ...
    kernel = [gaussian_kernel(sigma=std_max, separate=True, random=True, min_sigma=std_min, seed=s).astype(np.float32) for s in seeds]
    if isotropic:
...
if __name__ == '__main__':
    ...
    image = sitk.GetImageFromArray(image.data.cpu().numpy()[0, 0])
    sitk.WriteImage(image, 'test_img.nii')
