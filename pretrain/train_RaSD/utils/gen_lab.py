...
if __name__ == '__main__':
    ...
    im = draw_perlin(out_shape=[num_label, 1, *in_shape], scales=[32, 64, 128], max_std=1, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    warpped_im = draw_perlin(out_shape=[num_label, num_dim, *in_shape], scales=[32, 64, 128], max_std=16, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))

    warpped_im = transform(im, warpped_im)

    warpped_im = torch.argmax(warpped_im, dim=0)[0]
    warpped_im = warpped_im.data.cpu().numpy().astype(np.uint16)
    # cv2.imwrite('test.png', 255*(warpped_im[0]/np.max(warpped_im[0])))

    warpped_im = sitk.GetImageFromArray(warpped_im)
    sitk.WriteImage(warpped_im, 'test.nii')
