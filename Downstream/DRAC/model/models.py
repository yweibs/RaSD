...
from model.unet import UNetEncoderForClassification


def MedSAM(args, pretrained_path='medsam_vit_b.pth'):
    pretrained_path = os.path.join(args.pretrained_root, pretrained_path)
    from ref.TransUNet.vit_cls_modeling import ViTClassifier
    model = ViTClassifier(num_classes=args.out_channels,checkpoint_path=pretrained_path,pretrained=False)
...
def LVM_R50(args, pretrained_path='lvmmed_resnet.torch'):
    ...
    model.fc = nn.Sequential(
                        ...
                        nn.Linear(512, 128),
                        nn.ReLU(),
                        nn.Linear(128, args.out_channels))
...
def RaSE_2D(args, pretrained_path='RaSE_2D.pth'):
    pretrained_path = os.path.join(args.pretrained_root, pretrained_path)
    model = UNetEncoderForClassification(
            spatial_dims=2,
...
def scratch(args):
    ...
    # model_dict = torch.load(pretrained_path, map_location=torch.device('cpu'))
    # model = load(model, model_dict)
    print("Using RaSE_Scratch pretrained backbone weights !!!!!!!")
