...
def auto_bbox_from_mask(mask):
    pos = np.where(mask > 0)
    y_min, y_max = np.min(pos[0]), np.max(pos[0])
    x_min, x_max = np.min(pos[1]), np.max(pos[1])
    bbox = np.array([x_min, y_min, x_max, y_max])
    return torch.tensor(bbox)
    # results = np.empty((8, 4))
    # for i in range(mask.shape[0]):
    #     img = mask[i, 0]  # 形状为 [128, 128]
        
    #     # 获取不为0的像素的坐标
    #     y, x = np.where(img > 0)

    #     if len(x) > 0 and len(y) > 0:  # 确保有不为0的像素
    #         x_min = x.min()
    #         y_min = y.min()
    #         x_max = x.max()
    #         y_max = y.max()
    #     else:
    #         x_min = y_min = x_max = y_max = -1  # 处理全为0的情况

    #     results[i] = [x_min, y_min, x_max, y_max]
    # return results
