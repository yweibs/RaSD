...
class BaseTrainer(object):
    ...
    def get_training_phase(self):
        ...
        if self.config.pretrained_model is not None:
            ...
            if hasattr(model, 'swinViT'):
                self.transferred_dict = model.swinViT  # 或其他相关属性
            else:
                raise AttributeError("Model does not have the required attributes for fine-tuning.")
    ...
    def load_pretrained_weights(self):
        ...
        checkpoint = torch.load(self.config.pretrained_model, map_location=self.device, weights_only=False)

        # 打印检查点中的键
        # print("Checkpoint keys:", checkpoint.keys())

        # 直接使用检查点中的权重
        # pretrained_state_dict = checkpoint  # 直接使用 checkpoint
        ...
        #         k.replace(self.config.transferred_dismatched_keys[0], self.config.transferred_dismatched_keys[1]): v
