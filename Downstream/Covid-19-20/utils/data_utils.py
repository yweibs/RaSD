...
def get_loader(args):
    covid_dir = args.data_dir
    covid_json = args.json_list
    covid_train_list, covid_val_list = load_fold(covid_json, covid_dir)
    covid_cache_dir = args.cache_dir
    ...
    train_sampler = Sampler(covid_train_ds) if args.distributed else None
    ...
    # else:
    val_ds = data.Dataset(data=covid_val_list, transform=val_transform)
